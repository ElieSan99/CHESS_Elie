...
def retranscrire_caractere2(dict_caractere_joueur, jeu):
    ...
    for caractere in dict_caractere_joueur:
        ...
        # Trouver le caractère prédit correspondant à predicted_class_index
        top_prediction = None
        for cle, valeur in classes.items():
            if valeur == predicted_class_index:
                top_prediction = cle
        ...
        # Trouver le second caractère le plus probable
        second_best_index = np.argsort(res)[0][-2]  # Index du second meilleur résultat
        for cle, valeur in classes.items():
            if valeur == second_best_index:
                second_best_prediction = cle
                break

        # Construction du coup en utilisant la prédiction
    ...
    # Initialiser coup_corrige avec la valeur de coup initial
    coup_corrige = coup

    print(f"coup initial prédit EN : {coup_corrige}")

    # Tester si le coup est parmi les coups possibles
    if coup in coups_possibles:
        ...
        for coup2 in coups_possibles:
            similarite = difflib.SequenceMatcher(None, coup, coup2).ratio()
        ...
        coup_suggere = similarites[0][1] if similarites else coup  # Premier coup similaire
        print(f"coup suggéré : {coup_suggere}")

        # Trouver le caractère différent entre coup et coup_suggere
        for i, (char1, char2) in enumerate(zip(coup, coup_suggere)):
            if char1 != char2:
                # Remplacer le caractère incorrect par le second caractère le plus probable
                coup_corrige = coup[:i] + conversion[second_best_prediction] + coup[i+1:]
